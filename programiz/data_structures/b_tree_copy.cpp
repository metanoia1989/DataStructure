...
/**
 * B-tree 比之前学的AVL tree都要复杂不少。看了今天还是没有搞定。    
 * B-tree 的确实现复杂，看别人的代码理解都不太容易。
 */ 

// B-tree的结点
template <typename T>
struct BNode {
    BNode<T> **child; // 指向孩子节点的数组指针
    T *key; // 指向key的数组指针
    unsigned size; 
    bool leaf; // 是否是叶子节点
};
 ...
 * Btree异常类
 */ 
class BtreeException : std::exception {
public:
    explicit BtreeException(const std::string& message) : m_message(message) {};
...
public:
     ...
     * 构造函数
     * 
     * @param unsigned degree 树的最小度
     * @param bool(*)(T, T) key比较函数
     * @param void(*)(T) 打印key的函数指针
     */ 
    BTree(unsigned degree, bool (*)(T, T), void (*)(T) = nullptr);
    virtual ~BTree();
    void insert(T key);
    T remove(T key);
...
template <typename T>
BTree<T>::BTree(unsigned degree, bool (*compare)(T, T), void(*printK)(T))
{
    minDegree = degree;
    lessThan = compare;
    root = new BNode<T>();
...
void BTree<T>::insert(T key)
{
    ...
    if (root->size == 2 * minDegree - 1) {
        auto newRoot = new BNode<T>();
        initialzeNode(newRoot);
        newRoot->leaf = false;
